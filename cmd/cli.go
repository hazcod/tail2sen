...
func main() {
...

	{
		...
		if conf.Microsoft.Audit.UpdateTable {
			if err := sentinel.CreateTable(ctx, logger, conf.Microsoft.Audit.RetentionDays); err != nil {
	...
	//
	{
		...
		if conf.Microsoft.Network.UpdateTable {
			if err := sentinel.CreateTable(ctx, logger, conf.Microsoft.Network.RetentionDays); err != nil {
